...
class PointGenerator:
    ...
    def generate_random_points_in_polygon(
        ...
        print(
            f"Successfully generated {len(points)} random points using {chosen_method.value}"
        )
        return points_gdf
    ...
    def _rejection_sampling_points(
        self, geometry, k: int, max_attempts: Optional[int] = None
    ) -> List[Point]:
        """
        ...
        if len(points) < k:
            ...
            print(
                "Consider using a polygon with larger area or increasing max_attempts"
            )
    ...
    def _triangulation_points(self, geometry, k: int) -> List[Point]:
        ...
        try:
            ...
            for triangle in triangles:
                if geometry.contains(triangle.centroid) or geometry.intersects(
                    triangle
                ):
            ...
            for _ in range(k):
                ...
                triangle_idx = np.random.choice(
                    len(valid_triangles), p=triangle_weights
                )
    ...
    def generate_grid_points(
        self, polygon_gdf, spacing: float
    ) -> Optional[gpd.GeoDataFrame]:
        """
    ...
    def get_statistics(self, polygon_gdf, points_gdf) -> dict:
        ...
        stats = {
            ...
            "bbox_area": (bounds[2] - bounds[0]) * (bounds[3] - bounds[1]),
            "point_density": (
                len(points_gdf) / geometry.area if geometry.area > 0 else 0
            ),
            "bbox_efficiency": (
                geometry.area / ((bounds[2] - bounds[0]) * (bounds[3] - bounds[1]))
                if (bounds[2] - bounds[0]) * (bounds[3] - bounds[1]) > 0
                else 0
            ),
        }
...
def generate_random_points_in_polygon(
    ...
    method_enum = (
        GenerationMethod.REJECTION_SAMPLING
        if method == "rejection_sampling"
        else GenerationMethod.TRIANGULATION
    )
    generator = PointGenerator(method=method_enum)
    return generator.generate_random_points_in_polygon(
        polygon_gdf, k, max_attempts=max_attempts
    )

...
def diagnose_voronoi_issues():
    ...
    world = fetch_world_map(resolution="low", save_path="test_world.geojson")
    ...
    vor, voronoi_polygons, points = analyzer.create_voronoi_from_capitals(capitals, argentina)
    ...
    if circle_count > polygon_count:
        issues.append(f"Too many circles: {circle_count} circles vs {polygon_count} polygons")
    
    # Check 2: Do all capitals have regions?
    if len(voronoi_polygons) != len(capitals):
        issues.append(f"Mismatch: {len(capitals)} capitals but {len(voronoi_polygons)} regions")
    ...
    if coverage_ratio < 0.5:
        issues.append(f"Low coverage: Voronoi regions cover only {coverage_ratio:.1%} of country")
    elif coverage_ratio > 1.2:
        issues.append(f"Over-coverage: Voronoi regions cover {coverage_ratio:.1%} of country")
